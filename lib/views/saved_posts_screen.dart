import 'package:baseprogrammer/controllers/post_action_controller.dart';
import 'package:baseprogrammer/controllers/post_controller.dart';
import 'package:baseprogrammer/views/post_detail_screen.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class SavedPostsScreen extends StatelessWidget {
  const SavedPostsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final postController = Get.find<PostController>();
    final actionController = Get.put(PostActionController());

    return Scaffold(
      appBar: AppBar(
        title: const Text("Saved Posts 💾"),
        // actions: [
        //   // ✅ Clear All Button
        //   Obx(() {
        //     if (actionController.savedPosts.isEmpty) {
        //       return const SizedBox();
        //     }
        //     return IconButton(
        //       icon: const Icon(Icons.delete_sweep),
        //       tooltip: "Clear All Saved Posts",
        //       onPressed: () {
        //         Get.dialog(
        //           AlertDialog(
        //             title: const Text("Clear All Saved Posts?"),
        //             content: const Text(
        //                 "Do you want to remove all saved posts?"),
        //             actions: [
        //               TextButton(
        //                 onPressed: () => Get.back(),
        //                 child: const Text("Cancel"),
        //               ),
        //               TextButton(
        //                 onPressed: () {
        //                   actionController.clearAllSaved();
        //                   Get.back();
        //                 },
        //                 child: const Text(
        //                   "Clear All",
        //                   style: TextStyle(color: Colors.red),
        //                 ),
        //               ),
        //             ],
        //           ),
        //         );
        //       },
        //     );
        //   })
        // ],
      ),

      body: Obx(() {
        final savedIds = actionController.savedPosts;

        // ✅ Filter saved posts
        final savedPosts = postController.posts
            .where((post) => savedIds.contains(post.id))
            .toList();

        if (savedPosts.isEmpty) {
          return const Center(child: Text("No saved posts yet 💾"));
        }

        return ListView.builder(
          itemCount: savedPosts.length,
          itemBuilder: (context, index) {
            final post = savedPosts[index];

            return Card(
              margin: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              child: ListTile(
                leading: post.imageUrl.isNotEmpty
                    ? ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: Image.network(
                          post.imageUrl,
                          width: 70,
                          height: 70,
                          fit: BoxFit.cover,
                          errorBuilder: (context, error, stackTrace) =>
                              Container(
                                width: 70,
                                height: 70,
                                color: Colors.grey.shade200,
                                child: const Icon(Icons.broken_image, size: 24),
                              ),
                        ),
                      )
                    : const Icon(Icons.image),
                title: Text(
                  post.title,
                  maxLines: 2,
                  overflow: TextOverflow.ellipsis,
                ),
                subtitle: const Text("Tap to read"),

                // ✅ Remove Single Saved Post
                trailing: IconButton(
                  icon: const Icon(Icons.delete, color: Colors.red),
                  onPressed: () {
                    actionController.toggleSave(post.id);
                  },
                ),

                onTap: () => Get.to(() => PostDetailScreen(post: post)),
              ),
            );
          },
        );
      }),
    );
  }
}
